...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }} html {{ scroll-behavior:smooth; }} body {{ margin:0; background:var(--canvas); color:var(--ink); font:16px/1.5 "IBM Plex Sans",Arial,sans-serif; -webkit-font-smoothing:antialiased; }}
    ...
    main {{ padding-top:28px; padding-bottom:60px; }} .page-intro {{ margin-bottom:24px; }} .eyebrow {{ color:var(--green); font:700 10px "IBM Plex Mono",monospace; letter-spacing:.14em; text-transform:uppercase; }} h1 {{ font:700 38px/1.05 "Newsreader",Georgia,serif; letter-spacing:-.025em; margin:3px 0 4px; }} .intro {{ margin:0; color:var(--muted); font-size:14px; }}
    .section-head {{ display:flex; align-items:end; justify-content:space-between; gap:16px; margin:0 0 12px; }} .section-head h2 {{ margin:0; font:700 29px/1.1 "Newsreader",Georgia,serif; letter-spacing:-.02em; }} .section-meta {{ font:11px "IBM Plex Mono",monospace; color:var(--muted); white-space:nowrap; }}
    .sport-board {{ display:grid; grid-template-columns:40% 60%; border:1px solid var(--line); background:var(--surface); }} .sport-topic {{ display:grid; grid-template-columns:44px minmax(0,1fr); gap:12px; padding:19px 20px; border-bottom:1px solid var(--line); }} .sport-lead {{ grid-row:1 / span 4; background:var(--forest); color:#fff; padding:34px 28px; border:0; }} .sport-secondary:last-child {{ border-bottom:0; }}
    ...
    .topic-body h2 {{ font:700 20px/1.18 "Newsreader",Georgia,serif; margin:4px 0 5px; }} .sport-lead .topic-body h2 {{ font-size:36px; line-height:1.08; margin-top:18px; }} .topic-body h2 a {{ color:inherit; text-decoration:none; }} .topic-body h2 a:hover,.sport-row h3 a:hover {{ color:var(--green); text-decoration:underline; }} .sport-lead .topic-body h2 a:hover {{ color:#b9ebc8; }} .topic-body p {{ color:var(--muted); font-size:13.5px; margin:0 0 7px; }} .sport-lead .topic-body p {{ color:#e1f1e6; font-size:15px; margin:18px 0; }}
...
<body data-ts="{generated:.6f}">
  ...
  <main><div class="page-intro"><div class="eyebrow">Športový radar · posledných 24 hodín</div><h1>Šport</h1><p class="intro">Výber najrelevantnejších športových správ a úplný pracovný tok pre redakciu.</p></div>
